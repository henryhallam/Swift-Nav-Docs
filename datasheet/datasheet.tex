...
\mysubtitle{Flexible, high-performance open-source GPS receiver platform}
...
\begin{center}
  \begin{tikzpicture}[>=*]%,show background grid]
    ...
    \path (-0.1,1.875) coordinate (usb)
          ...
          (1.82,-0.05) coordinate (reset)
          (3.1,-0.05) coordinate (led1)
          (3.7,-0.05) coordinate (led2);
          
    \node [below left of=usb, node distance=2.5cm] (usb_label) {Micro-USB};
    \draw[->] (usb_label) -| (usb);
    
    \node [below of=usb_label] (rst_label) {Reset Button};
    \draw[->] (rst_label) -| (reset);
    
    \node [right of=rst_label, node distance=9.6cm] (led_label) {Status LEDs};
    \draw[->] (led_label) -| (led2);
    \draw[->] (led_label) -| (led1);
